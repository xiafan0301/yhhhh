  ...
  <div class="bg-release">
    ...
    <div class="bg-release-cot">
      <ul class="listxf" >
        ...
        <li><span class="title">附件</span><span class="content" v-if="!attachmentName">无</span><a class="content" :href="url" style="color:#0785FD;text-decoration:none;"> {{attachmentName.slice(0,20)}}<span v-if="attachmentName.length > 15">...</span><el-button  type="primary" style="margin-left: 20px" size="medium" v-if="attachmentName">下载</el-button></a><a target="_blank" :href="ajaxCtx2 + '/knowledgeBankService/previewAttach?' + 'where='+ encodeURIComponent(url)"><el-button style="margin-left: 20px; color: #0785FD" size="medium" v-if="attachmentName">预览</el-button></a></li>
        <li><span class="title">作者</span><span class="content" style="width: 80px">{{ seeKnowledgeList.author}}</span><span class="title">发布时间</span> <span class="content">{{seeKnowledgeList.publishTime | moment}}</span></li>
        <li><span class="title">上传用户</span><span class="content"  style="width: 80px">{{ seeKnowledgeList.uploadUser}}</span><span class="title">上传时间</span> <span class="content">{{seeKnowledgeList.createTime | moment}}</span></li>
      </ul>
    </div>

...
export default {
  ...
  methods: {
    ...
    // 获取筛选列表
    getAllGroups () {
      this.axios.get('S2/auth/authServices/userGroups', {params: {'where.proKey': this.$store.state.proKey}})
        .then(res => {
          if (res) {
    ...
    onConfirmDelete () {
      this.axios.delete('S2/auth/authServices/user/' + this.deleteItem.uid + '?proKey=' + this.$store.state.proKey)
        .then(res => {
          if (res) {

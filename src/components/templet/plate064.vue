...
<template>
  <div class="vis-temp-main" style="position: relative;">
    ...
    <div class="vis-cou-content" id="vis_temp_010"></div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    theReport (data) {
      ...
      let temp010 = document.getElementById('vis_temp_010');
      const chart = new G2.Chart({
        container: 'vis_temp_010',
        forceFit: true,
        padding: [ intRem, 8 * intRem, intRem, intRem ],
        width: G2.DomUtil.getWidth(temp010),
        height: G2.DomUtil.getHeight(temp010)
      });
      chart.source(dv, {
        percent: {
      ...
      chart.interval()
        ...
        .label('percent', {
          ...
          htmlTemplate: (text, item, index) => {
            ...
            return '<span class="title" style="display: inline-block;width: 50px;text-align: left;color:' + point.color + '; padding: .2rem .5rem;">' + point.reason + '</span><br>' +
